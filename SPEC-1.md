# GoldAtom/1: Claimant-Independent Scarcity Research

**DRAFT / NON-CONSENSUS / SUBJECT TO ADVERSARIAL REVISION**
...
GoldAtom/1 proceeds through a falsification-first sequence:

1. **Define competing geologies:** Bitcoin-single, Bitcoin-window, Bitcoin–Monero, and Bitcoin–Monero–Ethereum candidates receive explicit canonicalization and finality rules.
2. **Run one common attack harness:** test claimant grinding, source-miner withholding, reorganization, timestamp freedom, source stalls, weak-chain bribery, last-revealer power, and correlated control under identical assumptions.
3. **Compare and kill:** reject candidates that permit claimant-controlled supply amplification or whose added sources reduce effective security. Record failed candidates rather than erasing them.
4. **Select a surviving geology:** only a candidate that materially improves manipulation cost without discretionary off-chain judgment may advance.
5. **Implement after selection:** then build the Proof-Cast epoch miner/indexer and settlement prototype, including deterministic winner enumeration, ranked fallback, and reversible reorganization handling.

No miner or indexer should be written merely to discover which geology is preferable. Multi-chain complexity must earn its place under the attack harness rather than by making the object sound more geological.
